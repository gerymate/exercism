...
class Database:
    ...
    def get(self, table: str, key, primary_key="name"):
        """Search 'table' for an entry where 'primary_key' is equal to 'key'"""
        if table not in self.database:
            raise ValueError(f"No such table: {table}")
        data = self.database[table]
        for item in data:
            if item.get(primary_key) == key:
                return item
        return None

    def store(self, table: str, new_item, primary_key="name"):
        """Store 'new_item' in 'table' where 'primary_key' provides identity"""
        if table not in self.database:
            raise ValueError(f"No such table: {table}")
        data = self.database[table]
        for item in data:
            if item.get(primary_key) == new_item.get(primary_key):
